...
function App() {
  const [tasks, setTasks, { loading: tasksLoading, syncing: tasksSyncing, usingLocalStorage: tasksLocal }] = useCloudflareKV('tasks', []);
  const [tags, setTags, { syncing: tagsSyncing }] = useCloudflareKV('tags', ['Work', 'Personal']);
  const [priorities, setPriorities, { syncing: prioritiesSyncing }] = useCloudflareKV('priorities', ['Low', 'Medium', 'High']);
  const [theme, setTheme, { syncing: themeSyncing }] = useCloudflareKV('theme', 'light');
  ...
  const [filterByTags, setFilterByTags, { syncing: filterSyncing }] = useCloudflareKV('filterByTags', []);

  // Show loading indicator while initial data loads
  const isLoading = tasksLoading;
  const isSyncing = tasksSyncing || tagsSyncing || prioritiesSyncing || themeSyncing || filterSyncing;
    ...
    <div className={`app ${theme}`}>
      ...
      {isLoading ? (
        <div style={{
          ...
          <p>Loading from Cloudflare KV...</p>
        </div>
      ) : (
        <>
      <header className="app-header">
        ...
        <h1>To-Do List {tasksLocal && <span style={{fontSize: '0.6em', opacity: 0.7}}>(Local)</span>}</h1>
